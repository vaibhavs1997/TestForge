...
export const knowledgeService = {
  ...
  updateRuntimeVariable: async (projectId: string, variableId: string, payload: Partial<RuntimeVariableFormData>): Promise<RuntimeVariable> => {
    const { data } = await axios.patch(`${API_BASE}/projects/${projectId}/knowledge/variables/${variableId}`, payload);
    return data.data;
  },

  deleteRuntimeVariable: async (projectId: string, variableId: string): Promise<void> => {
    await axios.delete(`${API_BASE}/projects/${projectId}/knowledge/variables/${variableId}`);
  },

  // Dependencies
  listDependencies: async (projectId: string): Promise<Dependency[]> => {
    const { data } = await axios.get(`${API_BASE}/projects/${projectId}/knowledge/dependencies`);
    return data.data;
  },

  getDependency: async (projectId: string, dependencyId: string): Promise<Dependency> => {
    const { data } = await axios.get(`${API_BASE}/projects/${projectId}/knowledge/dependencies/${dependencyId}`);
    return data.data;
  },

  createDependency: async (projectId: string, payload: Omit<DependencyFormData, 'id' | 'projectId'>): Promise<Dependency> => {
    const { data } = await axios.post(`${API_BASE}/projects/${projectId}/knowledge/dependencies`, payload);
    return data.data;
  },

  updateDependency: async (projectId: string, dependencyId: string, payload: Partial<DependencyFormData>): Promise<Dependency> => {
    const { data } = await axios.patch(`${API_BASE}/projects/${projectId}/knowledge/dependencies/${dependencyId}`, payload);
    return data.data;
  },

  deleteDependency: async (projectId: string, dependencyId: string): Promise<void> => {
    await axios.delete(`${API_BASE}/projects/${projectId}/knowledge/dependencies/${dependencyId}`);
  },

  // Documentation
  listDocumentation: async (projectId: string): Promise<Documentation[]> => {
    const { data } = await axios.get(`${API_BASE}/projects/${projectId}/knowledge/docs`);
    return data.data;
  },

  getDocumentation: async (projectId: string, docId: string): Promise<Documentation> => {
    const { data } = await axios.get(`${API_BASE}/projects/${projectId}/knowledge/docs/${docId}`);
    return data.data;
  },

  createDocumentation: async (projectId: string, payload: Omit<DocumentationFormData, 'id' | 'projectId'>): Promise<Documentation> => {
    const { data } = await axios.post(`${API_BASE}/projects/${projectId}/knowledge/docs`, payload);
    return data.data;
  },

  updateDocumentation: async (projectId: string, docId: string, payload: Partial<DocumentationFormData>): Promise<Documentation> => {
    const { data } = await axios.patch(`${API_BASE}/projects/${projectId}/knowledge/docs/${docId}`, payload);
    return data.data;
  },

  deleteDocumentation: async (projectId: string, docId: string): Promise<void> => {
    await axios.delete(`${API_BASE}/projects/${projectId}/knowledge/docs/${docId}`);
  },
};
