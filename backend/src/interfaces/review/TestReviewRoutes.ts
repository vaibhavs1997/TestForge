...
export function createTestReviewRoutes(service: TestCaseVersionService): Router {
  const router = Router();
  ...
  router.get('/projects/:projectId/test-case-versions/:versionId/traceability', asyncHandler(async (req,res) => { service.assertVersionProject(req.params.versionId, req.params.projectId); res.json(createSuccessResponse(service.trace(req.params.versionId))); }));
