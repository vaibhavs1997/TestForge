...
export class GenerateRequirementsWithAI {
  ...
  async execute(request: GenerateRequirementsWithAIRequest): Promise<RequirementGenerationResult> {
    ...
    for (const input of requirementInputs) {
      ...
      // Persist using the existing RequirementRepository (also creates a Version entry)
      const saved = await this.requirementRepository.create(requirement);

      // Explicitly create a version entry for the AI-generated requirement.
      // (RequirementRepository.create already does this if versionService is wired,
      //  but we ensure it here for the AI-generation workflow.)
      try {
        await this.versionService.create({
          ...
          changeSummary: `Requirement generated by AI (${providerEntity.name})`,
          createdBy: 'AI',
        });
      } catch {
