...
function detectSpecType(content: string, fileName: string): SpecType {
  ...
  // .graphql → GraphQL schema text
  if (ext === 'graphql' || ext === 'gql') {
    return 'graphql-schema';
  }

  // JSON-based: try parsing and checking well-known fields
  if (ext === 'json') {
    try {
  ...
  // YAML-based: try parsing as YAML then checking
  if (ext === 'yaml' || ext === 'yml') {
    try {
...
function detectFromObject(obj: Record<string, unknown>): SpecType {
  ...
  // Postman Collection v2.1: has info + item
...
function parseJsonOrYaml(content: string, fileName: string): any {
  ...
  // For yaml/yml, try JSON first then fall back to YAML
  try {
  ...
  function processItems(items: any[]) {
    for (const item of items) {
      if (item.item && Array.isArray(item.item)) {
        // This is a folder — process recursively into same service
...
function parseGraphQLFields(body: string, typeName: string): ParsedOperation[] {
  ...
  // Split into field definitions. Each field is roughly: name(args): ReturnType
  ...
  for (const line of lines) {
    ...
    // Match: fieldName(arg: Type, ...): ReturnType  or  fieldName: ReturnType
  ...
  async execute(params: {
    ...
    try {
      if (specType === 'graphql-schema') {
        ...
        const spec = parseJsonOrYaml(params.content, params.fileName);
      ...
      return { servicesImported: 0, operationsImported: 0, duplicatesSkipped: 0, warnings };
    }
    ...
    for (const svc of parsedServices) {
      ...
      if (!exists) {
        ...
        serviceEntity = existing.find((s) => s.name.toLowerCase() === svc.name.toLowerCase()) ?? null;
        if (!serviceEntity) {
      ...
      for (const op of svc.operations) {
        const isDuplicate = existingOps.some(
          (e) => e.method === op.method && e.path === op.path
        );
        if (isDuplicate) {
          duplicatesSkipped++;
        } else {
    ...
    return { servicesImported, operationsImported, duplicatesSkipped, warnings };
  }
}
