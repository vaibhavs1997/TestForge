...
export class SecretResolutionService {
  constructor(private readonly secrets: SecretStore, private readonly projectId?: string) {}
  private async read(reference: string): Promise<string | null> {
    if (!this.projectId || (await this.secrets.metadata(reference))?.projectId !== this.projectId) throw new Error('Secret is not available in this project');
    return this.secrets.get(reference);
  }
  async resolve<T>(value: T): Promise<T> {
    if (isSecretReference(value)) { const secret = await this.read(value.secretRef); if (secret === null) throw new Error(`Secret ${value.secretRef} could not be resolved`); return secret as T; }
  ...
  async values(value: unknown): Promise<string[]> {
    if (isSecretReference(value)) { const secret = await this.read(value.secretRef); return secret === null ? [] : [secret]; }
