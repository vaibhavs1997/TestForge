...
export class GenerateExecutionPlanWithAI {
  ...
  async execute(request: GenerateExecutionPlanWithAIRequest): Promise<ExecutionPlanGenerationResult> {
    ...
    for (const input of planInputs) {
      ...
      // 17. Create Version entries
      try {
        await this.versionService.create({
          projectId,
          entityType: 'ExecutionPlan' as any,
          entityId: saved.id,
          snapshot: saved as any,
          changeSummary: `Execution plan generated by AI (${providerEntity.name}) for requirement ${requirement.title}`,
          createdBy: 'AI',
        });
      } catch {
