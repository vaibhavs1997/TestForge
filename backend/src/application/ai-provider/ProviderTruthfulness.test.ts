...
describe('provider truthfulness', () => {
  ...
  it('sanitizes health errors', async () => { const result = await new AIProviderResolutionService(new AIProviderRegistry()).health(entity('OpenAI')); expect(JSON.stringify(result)).not.toContain('secret-key'); });
});
