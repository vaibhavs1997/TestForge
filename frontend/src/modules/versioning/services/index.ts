...
export const versioningService = {
  listVersions: async (projectId: string, entityType?: EntityType, entityId?: string): Promise<Version[]> => {
    ...
    const { data } = await axios.get(`${API_BASE_URL}/projects/${projectId}/versions?${params.toString()}`);
    return data.data;
  },

  getVersion: async (versionId: string): Promise<Version> => {
    const { data } = await axios.get(`${API_BASE_URL}/versions/${versionId}`);
    return data.data;
  },

  getEntityVersions: async (projectId: string, entityType: string, entityId: string): Promise<Version[]> => {
    const { data } = await axios.get(`${API_BASE_URL}/projects/${projectId}/versions/${entityType}/${entityId}`);
    return data.data;
  },

  restoreVersion: async (projectId: string, versionId: string): Promise<Version> => {
    const { data } = await axios.post(`${API_BASE_URL}/projects/${projectId}/versions/${versionId}/restore`);
    return data.data;
  },

  compareVersions: async (projectId: string, versionId1: string, versionId2: string): Promise<VersionComparison> => {
    const { data } = await axios.get(`${API_BASE_URL}/projects/${projectId}/versions/compare/${versionId1}/${versionId2}`);
    return data.data;
  },
};
